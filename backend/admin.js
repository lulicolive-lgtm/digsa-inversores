...
// GET /api/admin/proxima — obtener datos de próxima inversión
...
// POST /api/admin/proxima — guardar datos de próxima inversión
...
// POST /api/admin/regenerar-reportes — regenerar todos los reportes (admin)
router.post('/regenerar-reportes', authMiddleware, adminOnly, async (req, res) => {
  try {
    const { generarYSubirReporte } = require('../utils/generarReporte');
    const { data: users } = await supabase
      .from('usuarios').select('id,nombre,apellido,email').eq('rol', 'inversor');
    
    const resultados = [];
    for (const user of (users || [])) {
      try {
        await generarYSubirReporte(user.id, user, supabase);
        resultados.push({ usuario: `${user.nombre} ${user.apellido}`, ok: true });
      } catch(e) {
        resultados.push({ usuario: `${user.nombre} ${user.apellido}`, ok: false, error: e.message });
      }
    }
...
// GET todas las notificaciones (admin)
...
router.post('/notificaciones', authMiddleware, adminOnly, async (req, res) => {
  ...
  try {
    if (usuario_id === 'todos') {
      const { data: inversores } = await supabase.from('usuarios').select('id').neq('rol','admin');
      for (const inv of (inversores||[])) {
        await supabase.from('notificaciones').insert([{ usuario_id: inv.id, titulo, mensaje, tipo }]);
      }
      res.json({ ok: true, enviadas: inversores?.length || 0 });
    } else {
