...
async function generarYSubirReporte(userId, usuario, sb = supabaseClient, extraPendiente = 0) {
  try {
    const [partRes, aportesRes, liqRes, pendientesRes] = await Promise.all([
      ...
      sb.from('liquidaciones').select('*, propiedades(nombre,precio_compra,precio_venta)').eq('usuario_id', userId).eq('publicado', true).order('fecha'),
      sb.from('pendientes_inversion').select('monto_eur').eq('usuario_id', userId).eq('asignado', false),
    ]);
    ...
    // Inversión inicial = aportes - retiros de capital (igual que Excel)
    ...
    // Pendiente = aportes tipo 'pendiente' en BD + extraPendiente pasado como parámetro
    const aportes_pendiente = aportes
      .filter(a => a.tipo === 'pendiente')
      .reduce((s, a) => s + Number(a.monto_eur || 0), 0);

    const pendiente = aportes_pendiente + extraPendiente > 0
      ? aportes_pendiente + extraPendiente
      : pisos_activos_raw.length === 0
    ...
    const pdfBuffer = await generarPDFReporte({
      ...
      fecha, inversion_inicial, valor_actual, rentabilidad_total, tir,
      pisos_activos, inversiones_finalizadas, aportes: aportesPDF, pendiente,
    });
    ...
    if (!uploadErr) {
      ...
      const meta = JSON.stringify({ inversion_inicial, valor_actual, rentabilidad_total, tir, pendiente, total_retornado });
      ...
      if (docExist) {
        ...
        await sb.from('documentos').insert([{
          usuario_id: userId, nombre: `Reporte de Inversión ${mes_año}`,
          tipo: 'reporte', url: urlData.publicUrl, fecha, publicado: true, metadata: meta,
        }]);
      }
    }
