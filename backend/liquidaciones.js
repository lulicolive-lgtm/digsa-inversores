...
router.post('/publicar/:propiedad_id', authMiddleware, adminOnly, async (req, res) => {
  ...
  try {
    ...
    // Publicar también los reportes generados (tipo reporte, sin propiedad_id)
    ...
    for (const l of (liqs || [])) {
      await supabase.from('notificaciones').insert([{
        ...
        mensaje: `Tu liquidación está lista. Retorno total: €${Math.round(l.total_retorno).toLocaleString('es-ES')}`,
        tipo: 'liquidacion'
      }]);
    }
...
router.post('/generar', authMiddleware, adminOnly, async (req, res) => {
  ...
  const { data: parts } = await supabase.from('participaciones')
    .select('*, usuarios(id,nombre,apellido,email)').eq('propiedad_id', propiedad_id).eq('activo', true);
  ...
  for (const part of parts) {
    ...
    try {
      const { data: todasLiqs } = await supabase.from('liquidaciones')
        .select('*, propiedades(nombre)').eq('usuario_id', user.id).order('fecha', { ascending: false });

      const liquidacionesParaPDF = (todasLiqs || []).map(l => {
        const ap = Number(l.aporte_usuario);
        const un = Number(l.utilidad_neta);
        const tir_anual = ap > 0 ? (un / ap) * 100 : 0;
        return {
          ...
          fecha_venta: l.fecha,
          precio_venta_piso: Number(l.precio_venta||0),
          porcentaje: Number(l.porcentaje||0),
          tir_anual,
        };
      });
      ...
      if (!uploadErr) {
        ...
        await supabase.from('documentos').insert([{
          ...
          nombre: `Liquidación ${prop.nombre} — ${iniciales}`,
          tipo: 'liquidacion', url: urlData.publicUrl, fecha, publicado: false
        }]);
      }
  ...
  // Marcar propiedad vendida y desactivar participaciones
  ...
  // Calcular % total asignado a pisos
  ...
  // Crear aporte pendiente si no hay destino O queda % sin asignar
  if (pct_pendiente > 0.001) {
    for (const res of resultados) {
      const monto_pend = Math.round(res.total_retorno * pct_pendiente * 100) / 100;
      await supabase.from('pendientes_inversion').insert([{
        usuario_id: res.usuario_id,
        propiedad_origen_id: propiedad_id,
        monto_eur: monto_pend,
        fecha,
        descripcion: `Liquidación ${prop.nombre} — pendiente de reinversión`,
        asignado: false
      }]);
    }
  }

  // Si hay destino → reinvertir el % asignado
  if (!sinDestino) {
    try {
      ...
      for (const res of resultados) {
        for (const propDest of (propsDest||[])) {
          ...
          const prop_pct = usarManual && destino_pcts[propDest.id]
            ? destino_pcts[propDest.id]
            : (totalCosto > 0 ? prop_costo / totalCosto : 1 / propsDest.length);
          ...
          await supabase.from('aportes').insert([{
            usuario_id: res.usuario_id, propiedad_id: propDest.id,
            monto_eur: monto, monto_usd: monto,
            fecha, tipo: 'aporte', descripcion: `Reinversión de ${prop.nombre}`
          }]);

          const { data: partExist } = await supabase.from('participaciones').select('id,monto_invertido,porcentaje')
            .eq('usuario_id', res.usuario_id).eq('propiedad_id', propDest.id).eq('activo', true).single();
          if (partExist) {
            await supabase.from('participaciones').update({
              monto_invertido: Number(partExist.monto_invertido) + monto,
              porcentaje: Number(partExist.porcentaje) + pct_nuevo
            }).eq('id', partExist.id);
          } else {
            await supabase.from('participaciones').insert([{
              usuario_id: res.usuario_id, propiedad_id: propDest.id,
              monto_invertido: monto, porcentaje: pct_nuevo,
              fecha_entrada: fecha, activo: true
            }]);
          }
        }
      }
    } catch(reinvErr) { console.error('Error reinversión:', reinvErr.message); }
  }

  // Generar reporte DESPUÉS de registrar pendiente/reinversión
  for (const res of resultados) {
    try {
...
// GET pendientes de inversión (admin)
...
router.post('/pendientes/asignar', authMiddleware, adminOnly, async (req, res) => {
  ...
  try {
    // Crear participación o sumar a existente
    const { data: partExist } = await supabase.from('participaciones')
      .select('id,monto_invertido,porcentaje').eq('usuario_id', usuario_id)
      .eq('propiedad_id', propiedad_id).eq('activo', true).single();
    
    const { data: prop } = await supabase.from('propiedades').select('precio_compra').eq('id', propiedad_id).single();
    const pct = prop?.precio_compra > 0 ? monto / Number(prop.precio_compra) : 0;

    if (partExist) {
      await supabase.from('participaciones').update({
        monto_invertido: Number(partExist.monto_invertido) + monto,
        porcentaje: Number(partExist.porcentaje) + pct
      }).eq('id', partExist.id);
    } else {
      await supabase.from('participaciones').insert([{
        usuario_id, propiedad_id, monto_invertido: monto,
        porcentaje: pct, fecha_entrada: fecha, activo: true
      }]);
    }

    // Registrar aporte
    await supabase.from('aportes').insert([{
      usuario_id, propiedad_id, monto_eur: monto, monto_usd: monto,
      fecha, tipo: 'aporte', descripcion: 'Reinversión desde pendiente'
    }]);

    // Marcar pendiente como asignado (o reducir monto)
    if (pendiente_id) {
