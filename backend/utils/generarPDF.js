...
async function generarPDFLiquidacion(data){
  ...
  const ROWS=13,TOTAL=1+Math.max(1,Math.ceil(liquidaciones.length/ROWS))+1;
  return new Promise((resolve,reject)=>{
    ...
    // Columnas exactas que suman CW=515
    const COLS=[
      {l:'Inmueble',w:130,a:'left'},
      {l:'Aporte',w:60,a:'right'},
      {l:'Util. bruta',w:60,a:'right'},
      {l:'Fee 15%',w:55,a:'right'},
      {l:'Imp. 25%',w:55,a:'right'},
      {l:'Util. neta',w:60,a:'right'},
      {l:'Liquidación',w:95,a:'right',b:true},
    ];
    const CCOLS=[null,null,null,C.naranja,null,C.verde,C.verde];

    let y;
    for(let i=0;i<liquidaciones.length;i++){
      if(i%ROWS===0){
        if(i>0)footer(doc,pg,TOTAL);
        doc.addPage();pg++;
        y=newPage(doc,'LIQUIDACIÓN DE INVERSIÓN',nombre,fecha);
        y=section(doc,y,i===0?`Detalle por inmueble — ${liquidaciones.length} operaciones`:'Detalle (continuación)');
        y=thead(doc,y,COLS);
      }
      const l=liquidaciones[i];
      y=trow(doc,y,COLS,[l.piso,fE(l.aporte),fE(l.utilidad_bruta),fE(l.fee),fE(l.impuestos),fE(l.util_neta),fE(l.total)],i%2===0?C.gris1:C.blanco,CCOLS);
    }

    // Total
    doc.rect(MAR,y,CW,17).fill(hex(C.header));
    let tx=MAR;
    [{t:'TOTAL',w:130},{t:fE(total_aporte),w:60,a:'right'},{t:'',w:60},{t:'',w:55},{t:'',w:55},{t:fE(total_utilidad),w:60,a:'right'},{t:fE(total_retorno),w:95,a:'right'}].forEach(c=>{
      doc.fillColor(hex(C.blanco)).font('Helvetica-Bold').fontSize(7.5).text(c.t,tx+2,y+4,{width:c.w-4,align:c.a||'left'});tx+=c.w;
    });
    y+=21;

    doc.rect(MAR,y,CW,24).fill(hex(C.gris1));
    doc.rect(MAR,y,3,24).fill(hex(C.azul));
    doc.fillColor(hex(C.subtxt)).font('Helvetica').fontSize(7.5).text('Rentabilidad NETA: fee de éxito 15% + impuestos 25%. Montos en euros (€).',MAR+8,y+7,{width:CW-16});
    footer(doc,pg,TOTAL);
