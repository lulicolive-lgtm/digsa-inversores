...
router.get('/todos', authMiddleware, adminOnly, async (req, res) => {
  let query = supabase.from('documentos').select('*, propiedades(nombre), usuarios(nombre,apellido)');
...
router.get('/', authMiddleware, async (req, res) => {
  let query = supabase.from('documentos').select('*, propiedades(nombre), usuarios(nombre,apellido)');
  if (req.user.rol !== 'admin') query = query.eq('usuario_id', req.user.id).eq('publicado', true);
  if (req.query.usuario_id) query = query.eq('usuario_id', req.query.usuario_id);
  const { data } = await query.order('created_at', { ascending: false });
  res.json(data || []);
});
...
router.post('/', authMiddleware, adminOnly, upload.single('archivo'), async (req, res) => {
  ...
  const { error: upErr } = await supabase.storage.from('documentos').upload(path, req.file.buffer, { contentType: req.file.mimetype, upsert: false });
  if (upErr) return res.status(500).json({ error: 'Error al subir archivo' });
  const { data: urlData } = supabase.storage.from('documentos').getPublicUrl(path);
  const { data, error } = await supabase.from('documentos').insert([{ usuario_id, propiedad_id: propiedad_id || null, nombre, tipo: tipo || 'reporte', url: urlData.publicUrl, fecha: fecha || null, publicado: true }]).select().single();
