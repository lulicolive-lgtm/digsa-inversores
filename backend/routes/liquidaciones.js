...
router.post('/generar', authMiddleware, adminOnly, async (req, res) => {
  ...
  if (!propiedad_id || !precio_venta || !fecha) return res.status(400).json({ error: 'propiedad_id, precio_venta y fecha son requeridos' });
  ...
  for (const part of parts) {
    ...
    } catch(pdfErr) { console.error('Error PDF:', pdfErr.message); }
